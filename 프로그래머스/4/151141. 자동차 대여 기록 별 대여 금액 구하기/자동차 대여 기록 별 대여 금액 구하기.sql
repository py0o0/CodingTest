    ...
    case
        ...
        when datediff(END_DATE, START_DATE) + 1 < 30 
            then round(DAILY_FEE * (datediff(END_DATE, START_DATE) + 1) * (
                ...
                where DURATION_TYPE like "7일%" and CAR_TYPE = "트럭"
                ), 0)
        when datediff(END_DATE, START_DATE) + 1 < 90
            then round(DAILY_FEE * (datediff(END_DATE, START_DATE) + 1) * (
                select (100 - DISCOUNT_RATE) / 100
                from CAR_RENTAL_COMPANY_DISCOUNT_PLAN
                where DURATION_TYPE like "30일%" and CAR_TYPE = "트럭"
                ), 0)
        else round(DAILY_FEE * (datediff(END_DATE, START_DATE) + 1) * (
            select (100 - DISCOUNT_RATE) / 100
            from CAR_RENTAL_COMPANY_DISCOUNT_PLAN
            where DURATION_TYPE like "90일%" and CAR_TYPE = "트럭"
            ), 0)
    end as FEE
from CAR_RENTAL_COMPANY_CAR a
    join CAR_RENTAL_COMPANY_RENTAL_HISTORY b
    on a.CAR_ID = b.CAR_ID
    and a.CAR_TYPE like "트럭"
